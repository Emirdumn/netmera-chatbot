...
def test_low_confidence_needs_brain():
    ...
    assert needs_brain_review(d, {}, "Merhaba") is True
...
def test_orchestrator_skips_brain_on_confident_control():
    ...
    control_decision = _decision(target_agent="support", confidence=0.92, action="continue")
    orch._control = MagicMock()
    orch._control.invoke.return_value = control_decision
...
def test_orchestrator_calls_brain_on_conflict():
    ...
    orch._control.invoke.return_value = control_decision
    orch._brain = MagicMock()
    orch._brain.invoke.return_value = brain_decision
    ...
    # sticky: is_answer + active_agent + not escalate → sales continue
